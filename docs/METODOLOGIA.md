...
**Mapa del espectro (2D/3D) y umbral de evidencia de entidades.** Los planos detallados cruzan los tres macro-ejes con respaldo académico (económico × GAL-TAN × territorial; ver docs/investigacion/ESPACIO-EJES.md) y sitúan a usuario, partidos y referencias doctrinales **con el mismo instrumento** (`proyectarEnEspacio` pasa las posiciones documentadas por `calcularFacetas`). La primera brújula reconocible Economía × Poder no sustituye GAL-TAN: su vertical compone, con pesos publicados en `COMPONENTES_AUTORIDAD_POLITICA`, sociedad 1,0; pluralismo institucional invertido 1,1; organización 1,2; libertades civiles invertidas 0,9; democracia directa invertida 0,25; estatismo 0,6; y tradición moral 0,4. Cada componente recibe un factor de evidencia `min(2, √n)`; se exigen al menos cuatro observaciones y dos familias de facetas. Así una corriente laica pero de partido único no aparece automáticamente como libertaria. Es una proyección editorial provisional que deberá recalibrarse tras el piloto, y las facetas originales permanecen visibles.

La vara de evidencia difiere: al usuario se le mide contra lo que ha recorrido (cobertura relativa); a una entidad, contra un mínimo absoluto de posiciones documentadas con carga en el eje (≥4 por macroeje, `UMBRAL_EVIDENCIA_ENTIDADES`), porque exigirle un porcentaje de toda la carga del banco la excluiría siempre. Una entidad sin evidencia suficiente para el par solicitado **no se dibuja** y el mapa declara cuántas quedaron fuera y por qué; la interfaz muestra el número de ítems que sostiene cada punto.
