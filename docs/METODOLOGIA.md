...
La vara de evidencia difiere: al usuario se le mide contra lo que ha recorrido (cobertura relativa); a una entidad, contra un mínimo absoluto de posiciones documentadas con carga en el eje (≥4 por macroeje, `UMBRAL_EVIDENCIA_ENTIDADES`), porque exigirle un porcentaje de toda la carga del banco la excluiría siempre. Una entidad que no alcance ni el umbral sólido ni el provisional partidista del par solicitado **no se dibuja** y el mapa declara cuántas quedaron fuera y por qué.
