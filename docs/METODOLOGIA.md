...
**Mapa del espectro (2D/3D) y umbral de evidencia de entidades.** Los planos detallados cruzan los tres macro-ejes con respaldo académico (económico × GAL-TAN × territorial; ver docs/investigacion/ESPACIO-EJES.md) y sitúan a usuario, partidos y referencias doctrinales **con el mismo instrumento** (`proyectarEnEspacio` pasa las posiciones documentadas por `calcularFacetas`). La primera brújula reconocible separa dos facetas directas: `propiedad-mercado` y `autoridad-politica`. El horizontal distingue propiedad/control productivo y coordinación por mercados sin convertir automáticamente proteccionismo o tamaño del Estado en «izquierda». La vertical suma cada pregunta una sola vez con cargas publicadas sobre contrapesos, libertades/coerción, jerarquía y participación; GAL-TAN, tradición moral y estatismo económico quedan fuera.

Los **65 partidos activos permanecen siempre como puntos** en esta brújula y en su selector. `data/brujula-partidos.json` fija una síntesis documental para el par de ejes y `data/brujula-partidos-anclas.json` conserva las anclas verificables contra cada ficha. No se imputa neutralidad por ausencia de datos, no se desplaza un partido para que encaje visualmente y ningún partido real ocupa ±100. El cálculo estricto sigue clasificando la evidencia como sólida, provisional, estimada o insuficiente; la interfaz traduce el último caso a una posición orientativa —o monotemática— con trazo distinto e incertidumbre amplia. Una coordenada sólida exige al menos seis grupos independientes, tres familias o subdimensiones y, en Poder, dos anclas de contrapesos o libertades. Estas coordenadas explican el paisaje, pero **no sustituyen el matching por respuestas compartidas ni alteran su ranking**.

**Contrato visual duro del primer atlas.** La [taxonomía maestra](investigacion/TAXONOMIA-MAPA-IDEOLOGIAS.md) y [`data/mapa-ideologias.json`](../data/mapa-ideologias.json) fijan la cuadrícula detallada aportada por el usuario como prior normativo de composición. La implementación debe preservar posiciones relativas, vecindades, densidad, continuidad y ocupación de cuadrantes; no puede reducirse a cuatro rótulos extremos ni recolocar corrientes por intuición. Las desviaciones superiores a 25 puntos se justifican y las superiores a 60 se revisan por duplicado. Este contrato no convierte la imagen en fuente factual: definiciones, preguntas y adaptaciones españolas requieren evidencia independiente. Tampoco autoriza copiar su bitmap, colores, iconos, memes, texto o contornos; el producto usa una composición propia, degradada, accesible y en español. Los puntos calculados permanecen separados de este prior y nunca se mueven para que «encajen» visualmente.

La vara de evidencia difiere: al usuario se le mide contra lo que ha recorrido (cobertura relativa); a una entidad, contra un mínimo absoluto de posiciones documentadas con carga en el eje (≥4 por macroeje, `UMBRAL_EVIDENCIA_ENTIDADES`), porque exigirle un porcentaje de toda la carga del banco la excluiría siempre. En los **tres planos detallados** y para las referencias doctrinales, una entidad que no alcance el umbral no se dibuja y el mapa declara cuántas quedaron fuera y por qué. La brújula principal de partidos aplica la excepción explícita anterior: mantiene los 65 puntos y comunica la incertidumbre en vez de borrar perfiles.
...
- **Núcleo** (siempre): exactamente 65 preguntas generales que posicionan en los ejes principales. El modo rápido termina en un perfil provisional y permite continuar al exhaustivo conservando respuestas.
...
**Etiquetas ideológicas: dos capas, ninguna en voz propia.** Cada ficha puede llevar (1) `autodescripcion`: cómo se define el propio partido, con sus palabras y fuente («anticapitalista» en los estatutos de Adelante Andalucía es un dato, no un juicio); y (2) `clasificacion`: etiquetas analíticas externas —«derecha radical», «rojipardismo», «neofascismo»— **siempre atribuidas** a politología o prensa de referencia, con la explicación llana del término y su fuente. La interfaz las presenta como lo que son: «se define como…» y «clasificado como… por…». Lo que el proyecto no hace nunca es etiquetar en voz editorial propia sin fuente: informar de las etiquetas existentes es información; asignarlas nosotros sería juicio.
...
- **Sesgo de evidencia-bandera en la codificación de entidades**: el mismo cherry-picking que afecta a la selección de ítems opera, por accidente, al codificar partidos — las posiciones fáciles de documentar son las características (todas ±2 en la dirección propia), y con pocas posiciones por eje el partido sale clavado en la esquina del mapa (PSOE en −83 económico con 6 ítems cuando los expert surveys lo sitúan en torno a −30). Controles: (1) al codificar se buscan activamente las posiciones **moderadoras** (los 0 con fuente, los ±1, los ítems contraestereotipo: la concertada que el PSOE mantiene, las pensiones que VOX no recorta); (2) el validador avisa de todo partido que caiga a ≥85 de un polo con todas sus posiciones en ±2; (3) calibración externa contra CHES (script `calibrar`) para que las posiciones no sean invento del codificador. Las referencias doctrinales quedan exentas del aviso: un tipo ideal puede vivir legítimamente en el polo.
