...
- [x] Última matriz limpia ejecutada después de detener todos los agentes: datos (39 referencias), TypeScript raíz/web, Vitest 68/68, build Vite 7, presupuestos, auditorías de dependencias, Playwright 20/20 y `git diff --check`.
...
- [x] Diff final auditado y publicación preparada en `claude/espectro-political-vaa-3d35l3` y `main`; ambas referencias deben apuntar al mismo commit validado.
...
- [x] Brújula degradada e interactiva: rótulos doctrinales ocultos por defecto y ficha genérica en español con definición, ejes, variante, periodo, cautelas y fuentes. La misma información se amplía dentro de las coincidencias doctrinales, sin duplicar tarjetas.

- [x] Dividir el bundle. Build de corte con 39 referencias: inicial 360 kB/104 kB gzip; resultados y catálogos 1.576 kB/362 kB gzip; 3D 926 kB/250 kB gzip; PWA completa 2,81 MiB. CI aplica presupuestos de 120/365/300 KiB gzip y 3,20 MiB totales. El límite de resultados se recalibró desde 350 KiB al crecer el catálogo, con solo ~3 % de margen.
...
- [x] Playwright cubre 20 flujos: teclado Likert, `Sin opinión`, ramas condicionales, recarga, fallo y recuperación de almacenamiento, rápido → provisional → exhaustivo, hito intermedio de 150 respuestas, 320 px, contexto electoral incompleto, partido monotemático, referencias sensibles sin gamificación, brújula vectorial interactiva, axe, recuperación 3D y PWA offline.
...
- [x] Datos políticos incluidos y versionados dentro de la app (`instrumento v3`, corte offline `2026-07-11`, caducidad local de 90 días).
...
- [x] Taxonomía maestra de la imagen: 178 etiquetas clasificadas A–F; 28 candidatas principales, referencias exhaustivas separadas de facetas, movimientos, contexto histórico y memes. La imagen inspira el diseño, no determina catálogo ni coordenadas.
